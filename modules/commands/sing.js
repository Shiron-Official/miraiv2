    ...
    try {
        ...
        if (args.join(" ").indexOf("https://") == 0) {
            ...
            try {
                ...
                const getms = (await axios.get(link.trim(), { responseType: "arraybuffer" })).data;
                fs.writeFileSync(path1, Buffer.from(getms, "utf-8"));
                if (fs.statSync(__dirname + `/cache/${rdus}.mp3`).size > 1164000000) return api.sendMessage('File cannot be sent because it is larger than 500MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${rdus}.mp3`), event.messageID);
